...
```powershell
.\.venv\Scripts\python.exe scripts\train_model.py --dataset human_lymph_node --model mlp --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
```
...
```powershell
.\.venv\Scripts\python.exe scripts\run_inference.py --dataset human_lymph_node --checkpoint outputs\models\human_lymph_node_mlp.pt --max-sc-cells 6000 --device cpu
```

5. 如果设备支持 GPU，可以把上面命令中的 `--device cpu` 改为 `--device cuda`。
...
如果是在单台 Linux 工作站或服务器上运行，推荐使用下面这一组命令：

```bash
./.venv/bin/python scripts/inspect_data.py --dataset human_lymph_node
./.venv/bin/python scripts/train_model.py --dataset human_lymph_node --model mlp --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
./.venv/bin/python scripts/train_model.py --dataset human_lymph_node --model spatial_gcn --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
./.venv/bin/python scripts/run_inference.py --dataset human_lymph_node --checkpoint outputs/models/human_lymph_node_mlp.pt --max-sc-cells 6000 --device cpu
```

如果机器具备可用 GPU，则可以将 `--device cpu` 改为 `--device cuda`。
