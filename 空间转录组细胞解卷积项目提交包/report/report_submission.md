...
本项目训练的主入口为 [scripts/train_model.py](C:/Coding/260402_spatial-cell-deconvolution/scripts/train_model.py)，其典型调用方式如下：

训练 `MLP-only`：

```bash
python scripts/train_model.py --dataset human_lymph_node --model mlp --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
```

训练 `Spatial GCN`：

```bash
python scripts/train_model.py --dataset human_lymph_node --model spatial_gcn --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
```

各主要参数含义如下：

- `--dataset`：选择数据集
- `--model`：选择模型类型，当前支持 `mlp` 和 `spatial_gcn`
- `--epochs`：训练轮数
- `--num-spots`：构造 pseudo-spot 的数量
- `--max-sc-cells`：参考数据中用于训练的最大细胞数
...
如果不依赖集群环境，在单台计算机上也可以完成训练和推理。推荐流程如下：

1. 创建虚拟环境并安装依赖
2. 使用 `inspect_data.py` 检查数据是否能够正确读取
3. 使用 `train_model.py` 分别训练 `MLP-only` 和 `Spatial GCN`
4. 使用 `run_inference.py` 在真实 ST 数据上生成预测结果和空间热图

单机运行示例如下：

```bash
python scripts/inspect_data.py --dataset human_lymph_node
python scripts/train_model.py --dataset human_lymph_node --model mlp --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
python scripts/train_model.py --dataset human_lymph_node --model spatial_gcn --epochs 10 --num-spots 1000 --max-sc-cells 6000 --device cpu
python scripts/run_inference.py --dataset human_lymph_node --checkpoint outputs/models/human_lymph_node_mlp.pt --max-sc-cells 6000 --device cpu
```

若设备有 GPU，则可将 `--device cpu` 替换为 `--device cuda`。
